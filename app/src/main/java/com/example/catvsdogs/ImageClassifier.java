...
public class ImageClassifier {
    ...
    public static String predict(Bitmap image) {
        ...
        Log.d("Naman", "datatype is predict: "+ outType);
        float[][] out = new float[1][2];
        interpreter.run(inpBuffer, out);
        Log.d("Naman", "output length is predict: " + out.length);

        Log.d("Naman", "output is predict: " + Arrays.toString(out[out.length-1]));
        if(out[0][0]>= out[0][1]){
            return "Cat : " + String.format("%.2f", out[0][0] * 100);
        }
        return "Dog : " + String.format("%.2f", out[0][1] * 100);
